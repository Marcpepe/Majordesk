...
var collectionHolderEleve = $('span.eleves');
var $addEleveLink = $('<button type="button" class="btn btn-info" class="add_eleve_link"><i class="icon-plus"></i> Ajouter un élève</button>');		
function addEleveForm(collectionHolderEleve, $addEleveLink) {
	var prototype = collectionHolderEleve.attr('data-prototype');
	var newFormEleve = prototype.replace(/__name__/g, collectionHolderEleve.children().length + 20);
	$addEleveLink.before(newFormEleve);
}
$(function() {
	collectionHolderEleve.append($addEleveLink);
	$addEleveLink.on('click', function(e) {
		e.preventDefault();
		addEleveForm(collectionHolderEleve, $addEleveLink);
	});
});
$(document).on('click', '.remove-eleve-btn', function() {
	$(this).parent().remove();
});
$(document).on('click', '.erase-eleve-btn', function() {
	$(this).parent().remove();
});
