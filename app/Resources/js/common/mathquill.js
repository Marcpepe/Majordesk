...
var Parser = P(function(_, _super, Parser) {
  ...
  var letter = Parser.letter = regex(/^[a-z]/); //regex(/^[a-z]/i); //CHANGED
...
var MathBlock = P(MathElement, function(_) {
  ...
  _.write = function(cursor, ch, replacedFragment) {
    var cmd;
    if (ch.match(/^[a-eg-z]$/)) //exclude f because want florin // majorclass : exclude A-Z
      cmd = Variable(ch);
...
LatexCmds.mathbb = bind(Style, '\\mathbb', 'span', 'class="bbold font"');
...
// var Vecteur =
LatexCmds.vec = P(MathCommand, function(_, _super) { // CHANGED
  _.ctrlSeq = '\\vec';
  _.htmlTemplate =
      '<span class="non-leaf vec-structure">'
	+   '<span class="non-leaf vec-content">&0</span>'
...
// var Angle =
LatexCmds.widehat = P(MathCommand, function(_, _super) { // CHANGED
  _.ctrlSeq = '\\widehat';
  _.htmlTemplate =
      '<span class="non-leaf widehat-structure">'
	+   '<span class="non-leaf widehat-content">&0</span>'
    +   '<span class="widehat-hat">^</span>'
    + '</span>'
  ;
  _.textTemplate = ['ang(', ')'];
});
...
var Norm =
LatexCmds.lnorm =
LatexCmds.rnorm = P(Paren, function(_, _super) {  // CHANGED // FIXME
  _.init = function() {
    _super.init.call(this, '\\Vert', '\\Vert');
  };
...
LatexCmds.editable = P(RootMathCommand, function(_, _super) {
  ...
  _.jQadd = function() {
	...
	self.jQ.addClass('input-eleve is-input no-sync case-maths');
...
LatexCmds.smalleditable = P(RootMathCommand, function(_, _super) {
  _.init = function() {
    MathCommand.prototype.init.call(this, '\\smalleditable');
  };
  ...
  _.jQadd = function() {
	...
	self.jQ.addClass('input-eleve small-input is-input no-sync case-maths');
...
LatexCmds.sdot = LatexCmds.cdot =
  bind(BinaryOperator, '\\cdot ', '&middot;'); // CHANGED
...
LatexCmds['≥'] = LatexCmds.ge = LatexCmds.geq = bind(BinaryOperator,'\\geq ','&ge;');  // CHANGED
...
// LatexCmds.mathbbN = LatexCmds.naturals = LatexCmds.Naturals = // CHANGED
  // bind(VanillaSymbol,'\\mathbb{N}','&#8469;');

// LatexCmds.mathbbP = // CHANGED
// LatexCmds.primes = LatexCmds.Primes =
// LatexCmds.projective = LatexCmds.Projective =
// LatexCmds.probability = LatexCmds.Probability =
  // bind(VanillaSymbol,'\\mathbb{P}','&#8473;');

// LatexCmds.mathbbZ = LatexCmds.integers = LatexCmds.Integers = // CHANGED
  // bind(VanillaSymbol,'\\mathbb{Z}','&#8484;');

// LatexCmds.mathbbQ = LatexCmds.rationals = LatexCmds.Rationals =
  // bind(VanillaSymbol,'\\mathbb{Q}','&#8474;');

// LatexCmds.mathbbR = LatexCmds.reals = LatexCmds.Reals =
...
// LatexCmds.mathbbH = LatexCmds.Hamiltonian = LatexCmds.quaternions = LatexCmds.Quaternions =
...
//space
LatexCmds.space = bind(BinaryOperator, '\\text{ }', ' ');
...
LatexCmds.circ = bind(VanillaSymbol, '\\circ ', '&#x2218;');  // CHANGED
...
LatexCmds.Vert = bind(VanillaSymbol,'\\Vert', '&spar;');
...
// LatexCmds.o = LatexCmds.O = // CHANGED
...
var latexMathParser = (function() {
  ...
  var controlSequence =
    regex(/^[^\\]/)
    .or(string('\\').then(
      regex(/^[a-z]+/i) // regex(/^[a-z]+/i) // CHANGED
      .or(regex(/^\s+/).result(' '))
      .or(any)
